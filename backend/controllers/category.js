...
exports.getCategoryPageDetails = async (req, res) => {
    try {
        ...
        // Fetch category shell
        const selectedCategory = await Category.findById(categoryId).lean()

        // Fetch published courses directly by category (do not rely on category.courses array)
        ...
        // attach courses onto category object for response
        if (selectedCategory) {
        ...
        // Handle the case when there are no published courses yet - keep 200 so UI can render empty state
        if (selectedCategory.courses.length === 0) {
            return res.status(200).json({
                ...
                message: "No published courses for the selected category yet.",
            })
        }
